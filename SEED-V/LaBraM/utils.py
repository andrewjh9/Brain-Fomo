...
class SEEDVLoader(torch.utils.data.Dataset):
    def __init__(self, data_folder_path, file_list, target_sampling_rate=200):
    ...
    def __getitem__(self, index):
        file_path = os.path.join(self.data_folder_path, self.files[index])
        with open(file_path, "rb") as f:
            sample_dict = pickle.load(f)
        eeg_data = sample_dict["X"]
        emotion_label = int(sample_dict["y"])
        eeg_tensor = torch.FloatTensor(eeg_data)

        return eeg_tensor, emotion_label


def prepare_SEEDV_dataset(processed_data_root_path, target_sampling_rate=200):

    train_folder = os.path.join(processed_data_root_path, "processed_train")
    val_folder = os.path.join(processed_data_root_path, "processed_eval")
    test_folder = os.path.join(processed_data_root_path, "processed_test")

    if not os.path.isdir(train_folder):
        raise FileNotFoundError(f"Training data folder not found: {train_folder}")
    if not os.path.isdir(val_folder):
        raise FileNotFoundError(f"Validation data folder not found: {val_folder}")
    if not os.path.isdir(test_folder):
        raise FileNotFoundError(f"Test data folder not found: {test_folder}")
    ...
    train_dataset = SEEDVLoader(
        data_folder_path=train_folder,
        file_list=train_files,
        target_sampling_rate=target_sampling_rate
    )
    val_dataset = SEEDVLoader(
        data_folder_path=val_folder,
        file_list=val_files,
        target_sampling_rate=target_sampling_rate
    )
    test_dataset = SEEDVLoader(
