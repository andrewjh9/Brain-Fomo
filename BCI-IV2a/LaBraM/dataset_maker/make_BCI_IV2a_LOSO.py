...
def convert_and_dump_windows(windows, metadata_df, subfolder, test_subject_id, desc="Processing windows"):
    """
    ...
    base_path = f"/path/to/data/BCI_IV2a_LOSO/Subject_{test_subject_id}"
    ...
    if os.path.exists(folder_path):
        ...
        print(f"Folder does not exists: '{folder_path}', creating one...")
    
    os.makedirs(folder_path, exist_ok=True)
    
    for idx, window in enumerate(tqdm(windows, desc=desc)):
        index = windows.indices[idx] if isinstance(windows, Subset) else idx # make sure to get the correct index
        ...
        filename = f"subject-{subject_id}_trial-{index}.pkl"
...
if __name__ == "__main__":
    ...
    for test_subject_id in all_subjects:
        ...
        convert_and_dump_windows(train_windows, metadata_df=metadata_train, subfolder="processed_train", test_subject_id=test_subject_id, desc="Processing training windows")
        convert_and_dump_windows(val_windows, metadata_df=metadata_val, subfolder="processed_eval", test_subject_id=test_subject_id, desc="Processing validation windows")
        convert_and_dump_windows(test_windows, metadata_df=metadata_test, subfolder="processed_test", test_subject_id=test_subject_id, desc="Processing test windows")
