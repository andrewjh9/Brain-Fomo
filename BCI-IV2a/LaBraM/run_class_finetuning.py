...
def get_dataset(args):
    ...
    elif args.dataset == 'BCI_IV2a':
        train_dataset, test_dataset, val_dataset = utils.prepare_BCI_IV2a_dataset(args.data_path)
        ch_names = ['FZ', 'FC3', 'FC1', 'FCZ', 'FC2', 'FC4', 'C5', 'C3', 'C1', 'CZ', 'C2', 'C4', 
            'C6', 'CP3', 'CP1', 'CPZ', 'CP2', 'CP4', 'P1', 'PZ', 'P2', 'POZ']
        
        # ch_names = ["FC3","FC1","FCZ","FC2","FC4","C5","C3","C1","CZ","C2","C4", 
...
def main(args, ds_init):
    ...
    epoch_of_max_val_accuracy = -1
    best_epoch_max_test_acc = 0.0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if max_accuracy < val_stats["accuracy"]:
                ...
                epoch_of_max_val_accuracy = epoch
                
                if max_accuracy_test > best_epoch_max_test_acc:
                    best_epoch_max_test_acc = max_accuracy_test
    ...
    if data_loader_val is not None and epoch_of_max_val_accuracy != -1:
        print(f"Best model (based on validation accuracy) achieved at epoch: {epoch_of_max_val_accuracy}")
        print(f"  Corresponding Test Accuracy: {max_accuracy_test:.2f}%")
        print(f"  Corresponding Validation Accuracy: {max_accuracy:.2f}%")
    elif data_loader_val is None:
        print("Validation was disabled, so 'best model epoch' based on validation is not applicable.")
    else:
        print("No improvement in validation accuracy was observed during training.")
