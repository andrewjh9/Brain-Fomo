...
class BCI_IV2a_Loader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=250):
        self.root = root
        self.files = files
...
def prepare_BCI_IV2a_dataset(root):
    # set random seed
    seed = 999
    np.random.seed(seed)
    ...
    train_dataset = BCI_IV2a_Loader(
        os.path.join(
            root, "processed_train"), train_files
    )
    test_dataset = BCI_IV2a_Loader(
        os.path.join(
            root, "processed_test"), test_files
    )
    val_dataset = BCI_IV2a_Loader(
        os.path.join(
...
def get_metrics(output, target, metrics, is_binary, threshold=0.5, is_regression=False):
    if is_regression:
        return get_regression_metrics(output, target, metrics)
    elif is_binary:
...
def get_regression_metrics(output, target, metrics):
    """
    Calculate regression metrics for the MoBI task.
    """
    results = {}
    ...
    if "r2_score" in metrics:
        results["r2_score"] = r2_score(target, output)
    
    if "pearson_correlation" in metrics:
        correlations = []
        for i in range(target.shape[1]):  # For each joint
            corr, _ = pearsonr(target[:, i], output[:, i])
            correlations.append(corr)
        results["pearson_correlation"] = np.mean(correlations)
