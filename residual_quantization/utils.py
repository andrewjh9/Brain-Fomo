...
def prepare_TUEV_dataset(root):
    ...
    val_dir = os.path.join(root, "processed_eval")
    test_dir = os.path.join(root, "processed_test")

    if os.path.exists(train_dir):
        train_files = os.listdir(train_dir)
        train_dataset = TUEVLoader(train_dir, train_files)
    ...
    if os.path.exists(val_dir):
        ...
        val_dataset = TUEVLoader(val_dir, val_files)
        print(f"Loaded {len(val_files)} validation samples.")
    else:
        print("Validation data not found. Skipping.")

    if os.path.exists(test_dir):
        test_files = os.listdir(test_dir)
        test_dataset = TUEVLoader(test_dir, test_files)
...
def prepare_TUAB_dataset(root):
    ...
    if os.path.exists(train_dir):
        ...
        train_dataset = TUABLoader(train_dir, train_files)
        print(f"Loaded {len(train_files)} training samples.")
    else:
        print("Training data not found. Skipping.")
    ...
    if os.path.exists(val_dir):
        val_files = os.listdir(val_dir)
        val_dataset = TUABLoader(val_dir, val_files)
        print(f"Loaded {len(val_files)} validation samples.")
    else:
        print("Validation data not found. Skipping.")
    ...
    if os.path.exists(test_dir):
        test_files = os.listdir(test_dir)
        test_dataset = TUABLoader(test_dir, test_files)
        print(f"Loaded {len(test_files)} test samples.")
    else:
        print("Test data not found. Skipping.")
