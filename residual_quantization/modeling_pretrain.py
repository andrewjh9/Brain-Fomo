...
class NeuralTransformerForMaskedEEGModeling(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, vocab_size=8192, embed_dim=200, depth=12,
                 ...
                 use_abs_pos_emb=True, use_rel_pos_bias=False, use_shared_rel_pos_bias=False, init_std=0.02, num_quantizers=6, **kwargs):
        super().__init__()
        ...
        # remember how many residual quantizers to predict:
...
class NeuralTransformerForMEM(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, vocab_size=8192, embed_dim=200, depth=12,
        ...
        self.student = NeuralTransformerForMaskedEEGModeling(EEG_size, patch_size, in_chans, out_chans, vocab_size,
                                                             ...
                                                             init_std, num_quantizers)

        #self.lm_head = nn.Linear(embed_dim, vocab_size)
        #fix:
        # one head per residual quantizer
        self.num_quantizers = num_quantizers
        ...
        for head in self.lm_heads:
            trunc_normal_(head.weight, std=init_std)
        ...
        #no longer have a single self.lm_head
    ...
    def forward(
            ...
            mode: str = "mlm",  # "mlm", "contrastive", "both"
    ):
        """
        Returns
        -------
        mlm           → logits_1, logits_2
        contrastive   → z1, z2
        both          → logits_1, logits_2, z1, z2
        """

        if bool_masked_pos is None:  # no mask = all False
        ...
        # ------------------------------------------------------------------
        # 1. Build the two stochastic views *once* so all modes can use them
        # ------------------------------------------------------------------
        feat_1 = self.student(  # view-1
            x, input_chans, bool_masked_pos,
            return_all_patch_tokens=True
        )  # (B, N+1, C)

        feat_2 = self.student(  # view-2
            x, input_chans, ~bool_masked_pos,
            return_all_patch_tokens=True
        )

        # ------------------------------------------------------------------
        # 2. Masked-token reconstruction branch (only if needed)
        # ------------------------------------------------------------------
        # if mode in ("mlm", "both"):
        #     logits_1 = self.lm_head(feat_1[:, 1:][bool_masked_pos])
        #     logits_2 = self.lm_head(feat_2[:, 1:][~bool_masked_pos])
        #fix:
        if mode in ("mlm", "both"):
            # masked features: [B*M, C]
            f1 = feat_1[:, 1:][bool_masked_pos]
            f2 = feat_2[:, 1:][~bool_masked_pos]
            # apply each head and stack → [B*M*Q, vocab_size]
            logits_1 = torch.cat([h(f1) for h in self.lm_heads], dim=0)
            logits_2 = torch.cat([h(f2) for h in self.lm_heads], dim=0)
        #end fix

        # ------------------------------------------------------------------
        # 3. Contrastive branch (only if needed)
        # ------------------------------------------------------------------
        if mode in ("contrastive", "both"):
            cls_1 = feat_1[:, 0]  # CLS token
            cls_2 = feat_2[:, 0]
            z1 = F.normalize(self.projection_head(cls_1), dim=1)
            z2 = F.normalize(self.projection_head(cls_2), dim=1)

        # ------------------------------------------------------------------
        # 4. Return according to mode
        # ------------------------------------------------------------------
        if mode == "mlm":
...
def labram_base_patch200_1600_8k_vocab(pretrained=False, **kwargs):  # 5M
    ...
    model = NeuralTransformerForMEM(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), vocab_size=vocab_size, num_quantizers=6, **kwargs)
