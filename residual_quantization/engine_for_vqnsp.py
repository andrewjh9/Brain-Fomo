...
def train_one_epoch(model: torch.nn.Module, 
    ...
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        for step, (batch) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            # hotfix
            EEG, label = batch  # unpack the batch
    ...
    # stat the codebook usage information
    if hasattr(model.module, 'quantize'):
        quantizer = model.module.quantize

        try:
            # ResidualQuantizer: iterate through .quantizers
            if hasattr(quantizer, 'quantizers'):
                # each q.embedding.cluster_size is [n_embed]
                sizes = [q.embedding.cluster_size.to(device) for q in quantizer.quantizers]
                cluster_sizes = torch.stack(sizes, dim=0)  # [num_quantizers, n_embed]

            # Single quantizer: direct embedding
            elif hasattr(quantizer, 'embedding'):
                cluster_sizes = quantizer.embedding.cluster_size.to(device)  # [n_embed]

            else:
                raise AttributeError("Unsupported quantizer type")

            zero_cnt = int((cluster_sizes == 0).sum())
        except Exception as e:
            print(f"[Warning] Could not access quantizer stats: {e}")
            zero_cnt = -1

        train_stat = {k: meter.global_avg for k, meter in metric_logger.meters.items()}
        train_stat['Unused_code'] = zero_cnt
        print(f"Unused code in codebook(s): {zero_cnt}")
        print(f"Unused code in codebook(s): {zero_cnt} out of {cluster_sizes.numel()} total")
        return train_stat
...
def evaluate(data_loader_list, model, device, log_writer=None, epoch=None, ch_names_list=None, args=None):
    ...
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        for step, (batch) in enumerate(metric_logger.log_every(data_loader, 10, header)):
            ...
            # hotfix
            images, label = batch  # unpack the batch
    ...
    if hasattr(model.module, 'quantize'):
        ...
        try:
            if hasattr(quantizer, 'quantizers'):
                sizes = [q.embedding.cluster_size.to(device) for q in quantizer.quantizers]
                cluster_sizes = torch.stack(sizes, dim=0)
            elif hasattr(quantizer, 'embedding'):
                cluster_sizes = quantizer.embedding.cluster_size.to(device)
            else:
                raise AttributeError("Unsupported quantizer type")
            zero_cnt = int((cluster_sizes == 0).sum())
        except Exception as e:
    ...
    try:
        # Case 1: ResidualQuantizer
        if hasattr(quantizer, 'quantizers'):
            cluster_sizes = [q.embedding.cluster_size for q in quantizer.quantizers]
            cluster_sizes = torch.stack(cluster_sizes)  # shape: [num_quantizers, n_embed]
            zero_cnt = (cluster_sizes == 0).sum().item()

        # Case 2: NormEMAVectorQuantizer
        elif hasattr(quantizer, 'embedding'):
            cluster_sizes = quantizer.embedding.cluster_size  # shape: [n_embed]
            zero_cnt = (cluster_sizes == 0).sum().item()

        else:
            raise AttributeError("Quantizer type not recognized")

    except Exception as e:
        print(f"[Warning] Could not access cluster sizes: {e}")
        zero_cnt = -1  # signal an issue
...
def calculate_codebook_usage(data_loader, model, device, log_writer=None, epoch=None, args=None):
    ...
    # statistic
    zero_cnt = int((codebook_cnt == 0).sum())
    print(f"STAT:  {zero_cnt} tokens ({(zero_cnt / codebook_num) * 100:.2f}%) never are used in this codebook.")

    # **Also** inspect quantizer cluster stats if desired:
    if hasattr(model.module, 'quantize'):
        _, unused_codes = utils.get_quantizer_codebook_stats(model.module.quantize, device)
        print(f"[After gather] Unused code entries: {unused_codes}")
